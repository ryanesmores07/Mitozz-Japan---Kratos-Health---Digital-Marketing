...
When an asset needs stronger composition, hierarchy, imagery, or motion direction than the standard flow would usually require, apply a frontend-quality escalation lens.
...
15. `brand/references/business-context/visual/template-mapping-rules.json` — resolve `template_set` and `slide_blueprint` directly from this file based on the calendar row's `Format`, `Content Pillar`, and `Objective`. Do not run the external script manually; read the rules and apply them inline.
16. `workflows/03B-visual-engine-preflight.md` — all visual-engine, source-lane, and support-layer decisions are defined here. Run this preflight mentally as part of building the brief, not as a separate step.
