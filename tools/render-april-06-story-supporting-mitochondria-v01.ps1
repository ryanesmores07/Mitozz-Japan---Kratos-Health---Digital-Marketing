...
$storyRoot = Join-Path $workspaceRoot "output/instagram/stories/2026-04-06-story-supporting-mitochondria-mini-guide-v01"
...
$frames = @(
    @{
        ...
        RightMeta = Decode-UnicodeEscapes "\u4eca\u65e5\u306e\u898b\u65b9"
        Headline = @(
            (Decode-UnicodeEscapes "\u652f\u3048\u308b\u3063\u3066\u3001"),
            (Decode-UnicodeEscapes "\u8db3\u3059\u3053\u3068\u3060\u3051\u3058\u3083\u306a\u3044\u3002")
        )
        Body = @(
            (Decode-UnicodeEscapes "\u307e\u305a\u306f\u6bce\u65e5\u306e\u6761\u4ef6\u3092"),
            (Decode-UnicodeEscapes "\u9759\u304b\u306b\u898b\u308b\u3002")
        )
        Type = "cover"
    },
    @{
        File = "frame-02.png"
        LeftMeta = Decode-UnicodeEscapes "\u65e5\u5e38\u8a33"
        RightMeta = Decode-UnicodeEscapes "\u898b\u308b\u5834\u6240"
        Headline = @(
            (Decode-UnicodeEscapes "\u898b\u308b\u306e\u306f"),
            (Decode-UnicodeEscapes "\u4f11\u307f\u65b9\u3001\u6e80\u305f\u3057\u65b9\u3001"),
            (Decode-UnicodeEscapes "\u52d5\u304d\u65b9\u3002")
        )
        Body = @(
            (Decode-UnicodeEscapes "\u4e00\u3064\u305a\u3064\u3067\u306f\u306a\u304f\u3001"),
            (Decode-UnicodeEscapes "\u6d41\u308c\u3068\u3057\u3066\u8aad\u3080\u3002")
        )
        Type = "body"
    },
    @{
        File = "frame-03.png"
        LeftMeta = Decode-UnicodeEscapes "\u4fdd\u5b58\u7248"
        RightMeta = Decode-UnicodeEscapes "\u4eca\u65e5\u306e\u6295\u7a3f\u3078"
        Headline = @(
...
foreach ($frame in $frames) {
    ...
    if ($frame.Type -eq "cover") {
        ...
        $labels = @(
            (Decode-UnicodeEscapes "\u4f11\u307f\u65b9"),
            (Decode-UnicodeEscapes "\u6e80\u305f\u3057\u65b9"),
            (Decode-UnicodeEscapes "\u52d5\u304d\u65b9")
        )
        $values = @(
            (Decode-UnicodeEscapes "\u571f\u53f0\u3092\u898b\u308b"),
            (Decode-UnicodeEscapes "\u6d41\u308c\u3092\u6574\u3048\u308b"),
            (Decode-UnicodeEscapes "\u30ea\u30ba\u30e0\u3092\u652f\u3048\u308b")
        )
        $columnWidth = 972 / 3
        $labelHeight = 34
        $labelValueGap = 16
    ...
    elseif ($frame.Type -eq "body") {
        ...
        $midTitles = @(
            (Decode-UnicodeEscapes "\u671d\u306e\u91cd\u3055\u3092\u898b\u308b"),
            (Decode-UnicodeEscapes "\u4e71\u308c\u65b9\u3092\u898b\u308b"),
            (Decode-UnicodeEscapes "\u4f59\u767d\u306e\u6709\u7121\u3092\u898b\u308b")
        )
        $midBodies = @(
            (Decode-UnicodeEscapes "\u7fcc\u65e5\u306b\u6b8b\u308b\u611f\u899a\u3092"),
            (Decode-UnicodeEscapes "\u98df\u3079\u65b9\u306e\u6ce2\u304b\u3089"),
            (Decode-UnicodeEscapes "\u52d5\u3051\u308b\u5e45\u304b\u3089")
        )
        ...
        Draw-CenteredParagraph -Graphics $graphics -Text (Decode-UnicodeEscapes "\u4e00\u3064\u3060\u3051\u76f4\u3059\u3088\u308a\u3001\u6d41\u308c\u3092\u3084\u3055\u3057\u304f\u8aad\u3080\u3002") -Font $cardBodyFont -Brush $softBrush -X 168 -Y 1330 -Width 744 -Height 42
    }
    else {
        Draw-StoryCardBase -Graphics $graphics -Width $width -Height $height
        $graphics.DrawString([string]$frame.LeftMeta, $metaFont, $blueBrush, 58, 48)
        ...
        Draw-CenteredParagraph -Graphics $graphics -Text (Decode-UnicodeEscapes "\u30d5\u30a3\u30fc\u30c9\u3067\u5168\u4f53\u3092\u898b\u308b") -Font $cardTitleFont -Brush $textBrush -X 126 -Y $closeTitleY -Width 828 -Height $closeTitleHeight
        $graphics.DrawLine($accentPen, 404, $closeRuleY, 676, $closeRuleY)
        Draw-CenteredParagraph -Graphics $graphics -Text (Decode-UnicodeEscapes "\u30df\u30c8\u30b3\u30f3\u30c9\u30ea\u30a2\u3092\u652f\u3048\u308b\u610f\u5473\u3092`n\u65e5\u5e38\u306e\u8a00\u8449\u3067\u898b\u76f4\u3059\u3002") -Font $cardBodyFont -Brush $softBrush -X 136 -Y $closeBodyY -Width 808 -Height $closeBodyHeight
    }
...
Write-Output "Rendered April 6 supporting mitochondria story to $storyRoot"
