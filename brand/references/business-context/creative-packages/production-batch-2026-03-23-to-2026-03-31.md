...
Use this file as the readable batch snapshot for the active March production window.

For the lean per-asset downstream handoff, use [post-calendar-layer-2026-03-23-to-2026-03-31.md](post-calendar-layer-2026-03-23-to-2026-03-31.md).

## Batch Rule

Every asset in this batch should stay inside the approved Mitozz production system:

- education uses `Set 1 + Set 2`
- product moments use `Set 3` only through the locked bottle workflow when pack truth matters
- lifestyle uses `Set 4`
- trust and reassurance use `Set 5`
- stories simplify the parent set instead of inventing a new visual world

## Shared References

Use these repeatedly across the batch when relevant:

- `feed-education-white-card-01.png`
- `set-3-background-plate-v01.png`
- `set-3-background-plate-v02b-clean-curtain-light.png`
- `set-4-lifestyle-v02-no-bottle.png`
- `set-5-trust-portrait-v01.png`
- `mitozz-bottle.jpg`
...
For `2026-03-23` and `2026-03-27`:

- keep the Education White Card system consistent
- vary card alignment and supporting cues so the grid does not feel duplicated
- use `Set 3` only when product relevance genuinely helps
...
- lead with `Set 5` warmth and credibility
- let `Set 1` handle structure and readability
- keep the tone reflective rather than corporate

## Priority

The remaining unfinished work in this batch is:

1. create the March 30 feed prompt
